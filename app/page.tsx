    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1">
        ...
        <section className="relative flex min-h-[85vh] flex-col items-center justify-center overflow-hidden px-4 text-center">
          {/* Degradados animados — solo transform/opacity, sin repaint */}
          <div aria-hidden className="pointer-events-none absolute inset-0 overflow-hidden">
            {/* Glow 1 — morado central, deriva diagonal */}
            <div className="animate-hero-glow-1 absolute left-1/2 top-0 h-[500px] w-[500px] rounded-full bg-lz-primary/10 blur-[110px] sm:h-[750px] sm:w-[750px] sm:blur-[130px]" />
            {/* Glow 2 — violeta inferior derecho, deriva horizontal */}
            <div className="animate-hero-glow-2 absolute bottom-0 right-1/4 h-[320px] w-[320px] rounded-full bg-lz-accent/7 blur-[90px] sm:h-[500px] sm:w-[500px] sm:blur-[110px]" />
            {/* Glow 3 — índigo inferior izquierdo, deriva vertical (solo sm+) */}
            <div className="animate-hero-glow-3 absolute -left-20 bottom-1/4 hidden h-[400px] w-[400px] rounded-full bg-lz-info/5 blur-[120px] sm:block" />
          </div>
              ...
              {categories.slice(0, 4).map((cat, i) => (
                <Reveal key={cat.slug} delay={i * 80}>
                  <Link
                    ...
                    className="group flex h-full flex-col gap-4 rounded-2xl border border-lz-border bg-lz-surface p-6 transition-all duration-200 hover:border-lz-primary/50 hover:shadow-[0_4px_24px_rgba(123,47,255,0.12)] active:scale-[0.98]"
                  >
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {BENEFITS.map((b, i) => (
              <Reveal key={b.title} delay={i * 80}>
                <div className="group flex h-full flex-col gap-4 rounded-2xl border border-lz-border bg-lz-surface p-6 transition-all duration-200 hover:border-lz-primary/30 hover:shadow-[0_2px_16px_rgba(123,47,255,0.08)]">
          ...
          <Reveal>
            <div className="relative mx-auto max-w-2xl px-4 py-24 text-center sm:px-6">
              ...
              <a
                href={`https://wa.me/${STORE_CONFIG.whatsappNumber}`}
                target="_blank" rel="noreferrer"
                className="inline-flex h-12 items-center gap-2 rounded-xl bg-[#25D366] px-8 text-sm font-semibold text-white transition-all hover:opacity-90 active:scale-[0.97]"
              >
