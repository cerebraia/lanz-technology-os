...
export function WhatsAppButton({ href, onClick, fullWidth, size = 'default' }: Props) {
  ...
  if (!href) {
    return (
      <div
        className={`flex items-center justify-center rounded-xl border border-lz-border bg-lz-surface px-4 py-3 text-xs text-lz-muted ${widthCls}`}
        role="status"
      >
        Contáctanos directamente para coordinar tu pedido
      </div>
    )
  }
  ...
  return (
    <a
      ...
      className={[
        ...
        'bg-[#25D366] transition-opacity hover:opacity-90 active:opacity-80',
        height,
        widthCls,
      ].join(' ')}
    >
