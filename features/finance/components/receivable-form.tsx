...
import { createReceivableAction, type FinanceActionState } from '@/features/finance/actions/finance-actions'

export function ReceivableForm() {
  const [state, formAction, pending] = useActionState<FinanceActionState, FormData>(
    createReceivableAction, undefined
  )
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <Input
        ...
        name="description"
        required
        placeholder="Venta pendiente, deuda cliente, etc."
      ...
      <div className="flex justify-end border-t border-lz-border pt-4">
        <Button type="submit" loading={pending}>Registrar cuenta por cobrar</Button>
      </div>
    </form>
  )
}
