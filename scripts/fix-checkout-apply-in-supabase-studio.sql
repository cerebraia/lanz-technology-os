-- ================================================================
-- FIX: Corrección crítica del checkout — Lanz Technology
-- Aplicar en: Supabase Studio → SQL Editor → Run
--
-- Bug: create_storefront_order llamaba reserve_inventory_movement
-- con v_order_id::text (texto) donde se esperaba uuid, causando
-- error 42883 "function does not exist" y failing silenciosamente
-- con "No se pudo crear el pedido. Intenta nuevamente."
--
-- Fix: remover ::text cast; pasar v_order_id como uuid directamente.
-- ================================================================
...
BEGIN
  ...
  INSERT INTO orders (
    ...
    v_order_number, v_customer_id, 'storefront', 'pending_confirmation', 'pending',
    p_currency, v_subtotal, 0, 0, 0, v_subtotal,
    NULLIF(trim(coalesce(p_notes,'')), ''),
    p_delivery_method, v_confirm_token, p_idempotency_key, NULL
  )
  ...
  FOR v_item_json IN SELECT * FROM jsonb_array_elements(v_validated_items) LOOP
    ...
    INSERT INTO order_items (
      ...
      v_item_json->>'sku', v_item_json->>'name',
      v_unit_price, v_reference_cost, coalesce(v_currency_code, p_currency),
      v_quantity, 0, v_line_total
    );

    -- FIX: v_order_id como uuid (no ::text) para que PostgreSQL resuelva
    -- la firma correcta de reserve_inventory_movement
...
-- Verificación inmediata (debe mostrar la función con fecha de hoy)
SELECT proname, prosrc LIKE '%p_reference_id   => v_order_id,%' AS fix_applied
FROM pg_proc
